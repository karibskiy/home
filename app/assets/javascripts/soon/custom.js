    ...
    **/
    ...
    function setDate(){
        ...
        if( launch < now ){
            ...
            message.html('Время настало...');
        }
        else{
            var s = -now.getTimezoneOffset()*60 + (launch.getTime() - now.getTime())/1000;
            var d = Math.floor(s/86400);
            days.html('<h1>'+d+'</h1><p>Дней'+(d>1?'':''),'</p>');
            s -= d*86400;

            var h = Math.floor(s/3600);
            hours.html('<h1>'+h+'</h1><p>Часов'+(h>1?'':''),'</p>');
            s -= h*3600;

            var m = Math.floor(s/60);
            minutes.html('<h1>'+m+'</h1><p>Минут'+(m>1?'':''),'</p>');

            s = Math.floor(s-m*60);
            seconds.html('<h1>'+s+'</h1><p>Секунд'+(s>1?'':''),'</p>');
            setTimeout(setDate, 1000);

            message.html('Время отсчитывает твои последние сухопутные часы');
        }
    }
})(jQuery);
/******************************************************************************************************************************
